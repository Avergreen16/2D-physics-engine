...
float get_cloud_density(vec3 a) {
    ...
    float density_falloff = smoothstep(0.0, 1.0, 1.0 - abs(fraction - ((0.5) / height)) / ((0.25) / height));

    float main_tex = sample_texture(fract(base_pos)).r;
    float tex = main_tex;
...
void main() {
    ...
    float step_size = max(0.01, dist_through / steps);
    ...
    while(step_dist < exit) {
        ...
        float light_step_dist = light_step_size * g;
        ...
        vec3 atmosphere_light = (atmosphere_density * scattering_coefficients * step_size) * a_light_transmittance * a_main_transmittance * c_light_transmittance * c_main_transmittance * atmosphere_phase;

        total_transmittance+= (cloud_density * absorbtion + atmosphere_density) * step_size;

        vec3 cloud_light = light_to_cloud * c_main_transmittance * a_main_transmittance * (cloud_density * absorbtion * step_size);
        if(c_main_transmittance < 0.2) cloud_light = vec3(0, 0, 0);
    ...
    total_transmittance = exp(-total_transmittance);
